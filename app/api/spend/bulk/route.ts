...
export async function POST(req: NextRequest) {
  ...
  const importSource = (body.importSource as ImportSource | undefined) ?? "PASTE";
  ...
  const bulkSource = importSource === "UPLOAD" ? "UPLOAD" : "BULK_IMPORT";
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Failed to add bulk rows";
    if (typeof message === "string" && message.includes("UPLOAD")) {
      return new Response(
        'Database enum is missing SpendSource.UPLOAD. Run Prisma migration, then retry.',
        { status: 500 },
      );
    }
